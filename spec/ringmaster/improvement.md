# Ringmaster — Improvement & Fine-Tuning Log

## Purpose

The improvement log is a persistent, structured record of every event in the system that indicates where the Ringmaster underperformed or where the model produced output the user rejected. It is entirely separate from the operational log (`ringmaster.log`) — different purpose, different audience, different retention policy.
...
- **Path:** configured via `logging.improve_log_path` in `config.yaml` — default `~/.ringmaster/improve.jsonl`
...
| `memory_forget` | `graph/nodes/memory.py` | Medium | User asked the assistant to forget something — signal that persist.py persisted something wrong or stale |
...
| `skill_proposed` | skills system (Phase 8) | Medium | Skill proposed by the assistant, awaiting approval — implicit rejection if never moved to approved/ |
...
{
  "node": "system",
  "proposed_action": "rm -rf /tmp/ringmaster-scratch",
  "payload_type": "command",
  "original_input": "clean up the ringmaster scratch directory"
}
```
